...
func newDesktopAppWithPaths(ctx context.Context, paths desktopPaths, log *slog.Logger, secrets secretKeyStore) (*DesktopApp, error) {
	...
	accounts, err := stagedAccounts(dataDir)
	if err != nil {
		return fail(err)
	}
	if err := store.ReconcileDiscoveredAccounts(ctx, accounts); err != nil {
		return fail(err)
	}
	if err := assignDiscoveredProxyModes(ctx, store, accounts); err != nil {
		return fail(err)
	}
	if err := store.EnsureAccounts(ctx, accounts); err != nil {
		return fail(err)
	}
...
func assignDiscoveredProxyModes(ctx context.Context, store *sqlite.ProductionStore, discovered []domain.Account) error {
	persisted, err := store.ListAccounts(ctx)
	if err != nil {
		return err
	}
	byID := make(map[domain.ID]domain.Account, len(persisted))
	for _, account := range persisted {
		byID[account.ID] = account
	}
	for index := range discovered {
		if existing, ok := byID[discovered[index].ID]; ok {
			discovered[index].ProxyMode = existing.ProxyMode
			discovered[index].ProxyProfileID = existing.ProxyProfileID
			continue
		}
		discovered[index].ProxyMode = domain.ProxyModeGlobal
		discovered[index].ProxyProfileID = nil
		discovered[index].Status = domain.AccountActive
	}
	return nil
}
...
func stagedAccounts(dataDir string) ([]domain.Account, error) {
	paths, err := filepath.Glob(filepath.Join(dataDir, "gotd-import-staging", "*", "*", "account-*", "session.json"))
	if err != nil {
		return nil, err
	}
	if len(paths) == 0 {
	...
	sort.Strings(paths)
	now := time.Now().UTC()
	accounts := make([]domain.Account, 0, len(paths))
	for index, path := range paths {
		if err := validateStagedSessionPath(dataDir, path); err != nil {
			return nil, err
		}
		accountID, driveImport, err := discoveredAccountID(dataDir, path)
		if err != nil {
			return nil, err
		}
		role := domain.AccountRoleSpammer
		if !driveImport && index == 2 {
			role = domain.AccountRoleScoutAnalyst
		}
		accounts = append(accounts, domain.Account{ID: accountID, Role: role, Status: domain.AccountStopped, SessionPath: path, ProxyMode: domain.ProxyModeUnassigned, CreatedAt: now, UpdatedAt: now})
	}
	return accounts, nil
}

func validateStagedSessionPath(dataDir, sessionPath string) error {
	...
	if err != nil || relative == ".." || strings.HasPrefix(relative, ".."+string(filepath.Separator)) {
		return errors.New("gotd session path must be beneath the data root")
	}
	parts := strings.Split(relative, string(filepath.Separator))
	if len(parts) < 2 {
		return errors.New("gotd session path is invalid")
	}
	ancestor := dataRoot
	for _, part := range parts[:len(parts)-1] {
		...
		if info.Mode()&os.ModeSymlink != 0 {
			return errors.New("gotd session path must not contain a symlink")
		}
		if !info.IsDir() {
			return errors.New("gotd session ancestor must be a directory")
		}
	}
	info, err := os.Lstat(path)
	if err != nil {
	...
	if !info.Mode().IsRegular() || info.Mode().Perm() != 0o600 {
		return errors.New("gotd session must be a 0600 regular file")
	}
	return nil
}

func discoveredAccountID(dataDir, sessionPath string) (domain.ID, bool, error) {
	relative, err := filepath.Rel(dataDir, sessionPath)
	if err != nil {
		return "", false, err
	}
	parts := strings.Split(filepath.ToSlash(relative), "/")
	if len(parts) == 5 && parts[0] == "gotd-import-staging" && parts[1] == "drive" &&
		strings.HasPrefix(parts[3], "account-") && parts[4] == "session.json" {
		index, parseErr := strconv.ParseUint(strings.TrimPrefix(parts[3], "account-"), 10, 32)
		if parseErr != nil || strings.TrimSpace(parts[2]) == "" {
			return "", false, errors.New("invalid Drive-imported Telegram session path")
		}
		return legacyDriveSessionAccountID(parts[2], uint32(index)), true, nil
	}
	digest := sha256.Sum256([]byte(filepath.ToSlash(relative)))
	return domain.ID(fmt.Sprintf("account-%x", digest[:6])), false, nil
}

func legacyDriveSessionAccountID(fileID string, accountIndex uint32) domain.ID {
	sum := sha256.Sum256([]byte(strings.TrimSpace(fileID) + "\x00" + strconv.FormatUint(uint64(accountIndex), 10)))
	return domain.ID("account-" + hex.EncodeToString(sum[:12]))
}
