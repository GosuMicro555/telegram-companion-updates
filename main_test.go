...
func TestProductionDesktopRestartPreservesCommittedDriveAccount(t *testing.T) {
	ctx := context.Background()
	root := t.TempDir()
	writeDesktopTorFixture(t, root)
	path := filepath.Join(root, "data", "drive-account-sessions", "import-fixture", "0.json")
	require.NoError(t, os.MkdirAll(filepath.Dir(path), 0o700))
	require.NoError(t, os.WriteFile(path, []byte("synthetic-session-state"), 0o600))
	db, err := sqlite.Open(ctx, filepath.Join(root, "data", "app.db"))
	require.NoError(t, err)
	credentials, err := newAccountCredentialStore(ctx, db, desktopTestSecrets{})
	require.NoError(t, err)
	candidate := driveaccounts.Candidate{
		Account:     domain.Account{ID: "tdata-fixture", DisplayName: "Fixture", Role: domain.AccountRoleSpammer, SessionPath: path},
		Credentials: messagecrypto.AppCredentials{AppID: 1, AppHash: "fixture"},
	}
	require.NoError(t, credentials.DriveAccountStore().CommitDriveAccounts(ctx, []driveaccounts.Candidate{candidate}))
	require.NoError(t, db.Close())
	before := snapshotExistingAccountArtifacts(t, root)
	for range 2 {
		...
		require.Equal(t, "tdata-fixture", accounts[0].ID)
		require.Equal(t, "Fixture", accounts[0].DisplayName)
		require.Equal(t, string(domain.AccountRoleSpammer), accounts[0].Role)
		app.Shutdown(ctx)
		require.Equal(t, before, snapshotExistingAccountArtifacts(t, root))
	}
}
