...
func TestInformationalNoUpdateAbortWaitsForCycleCompletion(t *testing.T) {
	source := readSource(t, "bridge_darwin_arm64.m")
	abortCallback := sourceBetween(t, source,
		"- (void)updater:(SPUUpdater *)updater didAbortWithError:",
		"- (void)updater:(SPUUpdater *)updater\n    didFinishUpdateCycleForUpdateCheck:",
	)

	for _, required := range []string{
		"self.action == TCSPActionCheck",
		"self.state == TCSPStatePending",
		"SUSparkleErrorDomain",
		"SUNoUpdateError",
		"return;",
	} {
		if !strings.Contains(abortCallback, required) {
			t.Errorf("informational no-update abort handling is missing %q", required)
		}
	}
	if strings.Index(abortCallback, "SUNoUpdateError") > strings.Index(abortCallback, "[self failWithCode:") {
		t.Fatal("informational no-update abort is failed before it can wait for Sparkle's cycle callback")
	}
	...
	if !strings.Contains(finishCallback, "error == nil || noUpdate") ||
		!strings.Contains(finishCallback, "self.state = TCSPStateDone") {
		t.Fatal("informational no-update must complete successfully only from Sparkle's cycle callback")
	}
}
